...
void ExtMergeSort::initial_pass() {
	...
	int fileID = 0;
	int *buf = new int[initial_buffer_size]; // 메모리(page_size) * buffer 개수 크기의 메모리 만듦
	while (IFS.read((char*) buf, sizeof(int) * initial_buffer_size)) {
		sort(buf, initial_buffer_size); // initial_buffer_size의 buffer를 sort
		stringstream ss_filename;
		ss_filename << "temp_0_" << fileID; //temp_0_0 temp_0_1 temp_0_2 temp_0_3 ~ 파일 생성
		write_array(ss_filename.str(), buf, initial_buffer_size); // 위 파일에 sort된  array가 각각 저장됨
		++fileID;
	}
	delete[] buf;
